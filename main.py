...
import json
from language_config import get_available_languages, get_language_mapping, save_language_config, load_language_config
...
app.config['PERMANENT_SESSION_LIFETIME'] = timedelta(hours=4)  # Sätt sessionens livslängd till 4 timmar
app.config['SESSION_PERMANENT'] = True
csrf = CSRFProtect(app)

ALLOWED_EXTENSIONS = {'csv'}

# Enkel användarstore - i produktion, använd en databas
users = {
    "admin": generate_password_hash("@admin!123@")
}

# Dekorator för inloggning krävs
...
def login():
    if request.method == 'POST':
        ...
        if username in users and check_password_hash(users[username], password):
            ...
            session.permanent = True  # Använd permanent session
...
def translate_descriptions():
    input_file = session.get('input_file')
    selected_languages = session.get('languages')
    dictionary_file = session.get('dictionary_file')
...
def upload_examples():
    ...
    if file and allowed_file(file.filename):
        ...
        try:
            ...
            available_languages = get_available_languages(df)
        ...
        return jsonify({'available_languages': available_languages})
